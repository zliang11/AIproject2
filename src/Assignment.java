...
public class Assignment {
    ...
    public static boolean isComplete(CSP c) {
        ...
        for(int i=0; i<c.variables.size(); i++) {
            if(!c.variables.get(i).valueAssigned()) {
                return false;
            }
        }
        return true;
...
//    }
    ...
    public static boolean ConsistentMap(CSP c, int i, String value) {//c, Variable position, domain
        c.variables.get(i).assignValue(value);//set assignment to this value (domain)
        return isConsistentMap(c);
    }

    public static boolean ConsistentJob(CSP c, int i, int v) {//c, position, domain
        c.variables.get(i).assignNum(v);//set assignment to this value (domain)
    ...
    public static boolean isConsistentMap(CSP c) {
        ...
        for(int i=0; i<c.constraints.size(); i++){
            Constraint constraint = c.constraints.get(i);
            Variable city1 = constraint.var1;
            Variable city2 = constraint.var2;

            if (city1.getAssignedValue() == city2.getAssignedValue() && city1.getAssignedValue() != null && city1.valueAssigned()){
                return false;
            }
        }
